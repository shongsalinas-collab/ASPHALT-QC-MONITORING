  ...
  md.forEach(lay=>{
    Object.values(lay.tests||{}).forEach(t=>{
      ...
      testRows.push('<tr>'+
        ...
        '<td>'+(lay.client||'--')+'</td>'+
        '<td>'+(lay.materials||'--')+'</td>'+
        '<td>'+(t.testType||'--')+(t.testingSource?'<br><span style="font-size:9px;color:'+(t.testingSource.includes('In-House')?'#3B6D11':'#378ADD')+'">'+(t.testingSource.includes('In-House')?'In-House':'Third Party')+'</span>':'')+'</td>'+
        '<td>'+(t.labNo||'--')+'</td>'+
        ...
        (t.testType==='Marshall Test'?(stab!==''&&stab!==undefined&&stab!==null?
          '<td class="result-cell"><span class="val" style="color:'+(parseFloat(stab)>=SPECS.stability.min?'#3B6D11':'#A32D2D')+'">'+parseFloat(stab).toFixed(2)+'</span>'+
          '<div class="req">min '+SPECS.stability.min+' kN</div></td>':
          '<td class="result-cell" style="color:var(--text-2)">--</td>'):'<td class="result-cell" style="color:var(--text-2)">--</td>')+
        ...
        '<td>'+(t.testingSource?'<span style="font-size:10px;padding:2px 7px;border-radius:6px;background:'+(t.testingSource==='In-House (Plant Laboratory)'?'#EAF3DE':'#EEF3FF')+';color:'+(t.testingSource==='In-House (Plant Laboratory)'?'#3B6D11':'#378ADD')+'">'+t.testingSource+'</span>':'--')+'</td>'+
        ...
        '<td>'+(t.remarks||'--')+'</td>'+
        '<td style="white-space:nowrap">'+
          '<button class="act-btn" onclick="editTest('+idx+',\''+t._key+'\')" title="Edit" style="color:#378ADD"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>'+
          '<button class="act-btn" onclick="delTest('+idx+',\''+t._key+'\')" title="Delete"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#A32D2D" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/></svg></button>'+
        '</td>'+
        '</tr>');
    });
  });

  tb.innerHTML=!testRows.length?'<tr class="empty-row"><td colspan="18">No test results found.</td></tr>':testRows.join('');
