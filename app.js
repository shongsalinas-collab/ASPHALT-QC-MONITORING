...
let SPECS = {
  ...
  jmfTargets: {
    ...
    'SMA - A': '',
    'SMA - B': '',
    'ATB - Asphalt Treated Basecoarse': ''
  }
};
...
function loadSpecsForm(){
  ...
  // Load JMF targets
  const jmf = SPECS.jmfTargets || {};
  ['Grading 310-B','Grading 310-C','Grading 310-D','Grading 310-E','SMA - A','SMA - B','ATB - Asphalt Treated Basecoarse'].forEach(function(mat){
    var id = 'jmf-' + mat.replace(/[ /-]/g,'_');
    sv(id, jmf[mat]||'');
  });
}
function saveSpecs(){
  SPECS={
    ...
    ,jmfTargets:(function(){ var j={}; ['Grading 310-B','Grading 310-C','Grading 310-D','Grading 310-E','SMA - A','SMA - B','ATB - Asphalt Treated Basecoarse'].forEach(function(mat){ var id='jmf-'+mat.replace(/[ /-]/g,'_'); j[mat]=gv(id)||''; }); return j; })()
  };
...
function evalResult(t){
  ...
  if(type==='Extraction Test'){
    ...
    if(!t.oacMat) return 'Pending';
    var oac=SPECS.jmfTargets&&SPECS.jmfTargets[t.oacMat]?parseFloat(SPECS.jmfTargets[t.oacMat]):NaN;
    if(isNaN(oac)) return 'Pending';
    var tol=SPECS.bitTol||0.4;
    var bit=parseFloat(t.bitumen);
    return (bit>=(oac-tol)&&bit<=(oac+tol))?'Passed':'Failed';
  }
...
function editTest(layIdx, testKey){
  try{
    ...
    sv('m-avavg',t.avAvg||'');sv('m-vmaavg',t.vmaAvg||'');
...
function updateTestResult(layIdx, testKey){
  ...
  const testEntry={
    ...
    bitumen:gv('e-bitumen'),oacMat:(testLayIdx!==null&&layings[testLayIdx]?layings[testLayIdx].materials:''),
...
function closeTestModal(){
  ...
  testLayIdx=null;
  const btn=document.getElementById('test-save-btn');
  var badge=document.getElementById('e-target-jmf-badge'); if(badge) badge.style.display='none';
...
function toggleTestFields(){
  ...
  computeResult();
  // Auto-fill OAC range for Extraction Test
  if(type==='Extraction Test' && testLayIdx!==null){
    var lay=layings[testLayIdx];
    var mat=lay?lay.materials:'';
    var jmf=SPECS.jmfTargets||{};
    var oac=mat&&jmf[mat]?parseFloat(jmf[mat]):NaN;
    var tol=SPECS.bitTol||0.4;
    var badge=document.getElementById('e-target-jmf-badge');
    var rangeEl=document.getElementById('e-target-range');
    if(!isNaN(oac)){
      var lo=(oac-tol).toFixed(2), hi=(oac+tol).toFixed(2);
      if(rangeEl){ rangeEl.textContent=lo+' – '+hi+'%'; rangeEl.style.display=''; }
      if(badge) badge.style.display='';
    } else {
      if(rangeEl){ rangeEl.textContent='--'; rangeEl.style.display='none'; }
...
function computeResult(){
  const type=gv('t-type');
  const t={testType:type,stabAvg:gv('m-stabavg'),flowAvg:gv('m-flowavg'),avAvg:gv('m-avavg'),vmaAvg:gv('m-vmaavg'),vfbAvg:gv('m-vfbavg'),bitumen:gv('e-bitumen'),oacMat:(testLayIdx!==null&&layings[testLayIdx]?layings[testLayIdx].materials:''),airVoids:gv('a-av'),vma:gv('a-vma'),vfb:gv('a-vfb'),coreCount:gv('c-count')};
...
function saveTestResult(){
  ...
  const coreReadings=[];
  for(let i=1;i<=count;i++){const e=document.getElementById('core-'+i);if(e&&e.value) coreReadings.push(parseFloat(e.value));}
  const lay2=layings[testLayIdx];
  const computedTl=lay2?tlStatus(lay2.dateLaid,date):'Pending';
  const testEntry={
    testType:type, testingSource:gv('t-source'), sampleSource:gv('t-samplesource'),
    dateTested:date, labNo:gv('t-labno').trim(), timeliness:computedTl,
        av1:gv('m-av1'),av2:gv('m-av2'),av3:gv('m-av3'),avAvg:gv('m-avavg'),
    vma1:gv('m-vma1'),vma2:gv('m-vma2'),vma3:gv('m-vma3'),vmaAvg:gv('m-vmaavg'),
    vfb1:gv('m-vfb1'),vfb2:gv('m-vfb2'),vfb3:gv('m-vfb3'),vfbAvg:gv('m-vfbavg'),
    stab1:gv('m-stab1'),stab2:gv('m-stab2'),stab3:gv('m-stab3'),stabAvg:gv('m-stabavg'),
